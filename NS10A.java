...
public class NS10A{
  ...
  void degreeCorrelation(){ // (7.7)
	...
	for (int i = 0; i <= kmax; i++)
		for (int j = 0; j <= kmax; j++) e[i][j] = 0;
	for (int i = 0; i < N; i++){
		int d1 = degrees[i];
		for (int j: neighbors.get(i)) e[d1][degrees[j]]++;
	}
	for (int i = 0; i <= kmax; i++){
		q[i] = 0;
		for (int j = 0; j <= kmax; j++) q[i] += e[i][j];
		knn[i] = 0;
		if (q[i] > 0){
			for (int j = 0; j <= kmax; j++) knn[i] += j * e[i][j];
			knn[i] /= q[i];
		}
	}
  }
  ...
  void printKnn(){
	for (int i = 0; i <= kmax; i++) if (knn[i] > 0) System.out.println(i + "\t" + knn[i]);
  }
