// NS9A.java 
// usage: java NS9A N m
// Barabasi-Albert model with number of nodes N and number of links m per new node
// m0 is set to be m + 2
// uses the last 50 time steps a dt to get a snapshot of degrees in degreeSnapshot
// then cumulative() computes dk/dt = m Pi(k) and thus Pi(k) = dk /dt / m and then average
// over all nodes with degree k in the earlier snapshot and then normalized
// pi(k) is then output along with k
...
class NS9A{
    ...
    public NS9A(int n, int mm){
        N = n; m0 = mm + 2; m = mm;
    }

    void evolve(){
	ArrayList<Integer> possibilities = new ArrayList<Integer>();  // one for each link end
	for (int i = 0; i < N; i++) neighbors.add(new HashSet<Integer>());
	neighbors.get(0).add(1); neighbors.get(1).add(0);
	possibilities.add(0); possibilities.add(1);
        Random random = new Random();
        for (int i = 2; i < m0; i++){  // initial links
        	int j = random.nextInt(i);
		neighbors.get(i).add(j); neighbors.get(j).add(i);
		possibilities.add(i); possibilities.add(j);
        }
	int j = 0;
	degreeSnapshot = new int[N - dt];
        for (int i = m0; i < N; i++){  // each node after m0 has m links
        	int randomBaseSize = possibilities.size();
        	for (int l = 0; l < m; l++){
			j = possibilities.get(random.nextInt(randomBaseSize)); // PA
			neighbors.get(i).add(j); neighbors.get(j).add(i);
			possibilities.add(i); possibilities.add(j);
        	}
  ...
  public static void main(String[] args){
   if (args.length < 2){
     System.err.println("Usage: java NS9A N m");
     System.exit(1);
   }
   NS9A ns9 = new NS9A(Integer.parseInt(args[0]), Integer.parseInt(args[1]));
   ns9.evolve();
   ns9.cumulative();
 }
}