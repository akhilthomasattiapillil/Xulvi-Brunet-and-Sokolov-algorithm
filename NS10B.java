...
class NS10B{
  ...
  void rewire(){
	...
	while (unswapped > 1){
		...
		int d = j - 1; 
		swapped[c] = swapped[d] = true; unswapped -= 2;
		if (edges[c][0] == edges[d][0] || edges[c][0] == edges[d][1]
			|| edges[c][1] == edges[d][0] || edges[c][1] == edges[d][1]) continue;
		...
		if (u == 3){
			int t = edges[c][1]; edges[c][1] = edges[d][1]; edges[d][1] = t;
		}else{
			int t = edges[c][1]; edges[c][1] = edges[d][0]; edges[d][0] = t;
		}
		// for rewiring for disassortativity, 
		// u should be the sum of the two nodes with less extreme degrees of the four
		/*if (u == 3)continue;  // already dassortative
		if (u == 1 || u == 5){
			int t = edges[c][0]; edges[c][0] = edges[d][0]; edges[d][0] = t;
		}else{
			int t = edges[c][0]; edges[c][0] = edges[d][1]; edges[d][1] = t;
		}*/
